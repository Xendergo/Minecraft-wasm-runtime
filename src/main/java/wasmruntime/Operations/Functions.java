...
public class Functions {
  ...
  public static void callIndirect(ValueStack stack) {
    ...
    if (table.values.length <= i32(stack.peek())) throw new TrapRuntime("call_indirect tried to access the index " + i32(stack.peek()) + " which isn't in the table");
    int funcIndex = funcref(table.values[i32(stack)]);
